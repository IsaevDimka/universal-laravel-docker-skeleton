...
Для получения списка категорий нужно вызвать метод **parseToArray** класса **\IsaevDimka\RussianPost\CategoryList**
```php
<?php

declare(strict_types=1);
  $CategoryList = new \IsaevDimka\RussianPost\CategoryList();
...
try {
  ...
  $TariffCalculation = new \IsaevDimka\RussianPost\TariffCalculation();
  $calcInfo = $TariffCalculation->calculate($objectId, $params, $services);
}

catch (\IsaevDimka\RussianPost\Exceptions\RussianPostTarrificatorException $e) {
    // Обработка ошибок тарификатора 
    $errors = $e->getErrors(); // Массив вида [['msg' => 'текст ошибки', 'code' => код ошибки]]
}

catch (\IsaevDimka\RussianPost\Exceptions\RussianPostException $e) {
...
?>
```
**$calcInfo** - это объект класса *IsaevDimka\RussianPost\CalculateInfo*
Доступные методы:
...
Массив тарифов состоит из объектов класса *IsaevDimka\RussianPost\Tariff*
Доступные методы:
 - *getId()* - ID тарифа
...
declare(strict_types=1);
    ...
    $otpravkaApi = new \IsaevDimka\RussianPost\Providers\OtpravkaApi(Yaml::parse(file_get_contents('path_to_config.yaml')));
    $otpravkaApi->setLogger($log);
    
    $addressList = new \IsaevDimka\RussianPost\AddressList();
    ...
    $Tracking = new \IsaevDimka\RussianPost\Providers\Tracking('single', $config);
...
Для работы используется экземпляр класса *IsaevDimka\RussianPost\Providers\Tracking*.  
...
declare(strict_types=1);
  ...
  use IsaevDimka\RussianPost\Providers\Tracking;
...
В случае возникновеня ошибок при обмене выбрасывает исключение *\IsaevDimka\RussianPost\Exceptions\RussianPostException*
...
declare(strict_types=1);
  use Symfony\Component\Yaml\Yaml;
  use IsaevDimka\RussianPost\Providers\OtpravkaApi;
...
?>
```
**$addressList** - это объект класса *IsaevDimka\RussianPost\AddressList* содержащий список адресов для нормализации.
...
declare(strict_types=1);
  ...
  try {
      ...
      $fioList = new \IsaevDimka\RussianPost\FioList();
...
?>
```
**$fioList** - это объект класса *IsaevDimka\RussianPost\FioList* содержащий список ФИО для нормализации.
...
declare(strict_types=1);
  ...
  try {
      ...
      $phoneList = new \IsaevDimka\RussianPost\PhoneList();
...
?>
```
**$phoneList** - это объект класса *IsaevDimka\RussianPost\PhoneList* содержащий список номеров телефлонов для нормализации.
...
declare(strict_types=1);
  use Symfony\Component\Yaml\Yaml;
  use IsaevDimka\RussianPost\Providers\OtpravkaApi;
  use IsaevDimka\RussianPost\ParcelInfo;
  ...
  try {
      ...
      /*
       IsaevDimka\RussianPost\TariffInfo Object
       (
           [totalRate:IsaevDimka\RussianPost\TariffInfo:private] => 30658
           [totalNds:IsaevDimka\RussianPost\TariffInfo:private] => 6132
           [aviaRate:IsaevDimka\RussianPost\TariffInfo:private] => 0
           [aviaNds:IsaevDimka\RussianPost\TariffInfo:private] => 0
           [deliveryMinDays:IsaevDimka\RussianPost\TariffInfo:private] => 1
           [deliveryMaxDays:IsaevDimka\RussianPost\TariffInfo:private] => 3
           [fragileRate:IsaevDimka\RussianPost\TariffInfo:p rivate] => 7075
           [fragileNds:IsaevDimka\RussianPost\TariffInfo:private] => 1415
           [groundRate:IsaevDimka\RussianPost\TariffInfo:private] => 30658
           [groundNds:IsaevDimka\RussianPost\TariffInfo:private] => 6132
           [insuranceRate:IsaevDimka\RussianPost\TariffInfo:private] => 0
           [insuranceNds:IsaevDimka\RussianPost\TariffInfo:private] => 0
           [noticeRate:IsaevDimka\RussianPost\TariffInfo:private] => 0
           [noticeNds:IsaevDimka\RussianPost\TariffInfo:private] => 0
           [oversizeRate:IsaevDimka\RussianPost\TariffInfo:private] => 0
           [oversizeNds:IsaevDimka\RussianPost\TariffInfo:private] => 0
       )
       */
  }
...
?>
```
**$parcelInfo** - это объект класса *IsaevDimka\RussianPost\ParcelInfo* содержащий данные по отправлению.
**$tariffInfo** - это объект класса *IsaevDimka\RussianPost\tariffInfo* содержащий данные по расчитанному тарифу.
...
declare(strict_types=1);
  ...
  try {
      ...
      /*
       IsaevDimka\RussianPost\TariffInfo Object
       (
           [functionalityCheckingRate:IsaevDimka\RussianPost\TariffInfo:private] => 30658
           [functionalityCheckingNds:IsaevDimka\RussianPost\TariffInfo:private] => 6132
           [withFittingRate:IsaevDimka\RussianPost\TariffInfo:private] => 0
           [withFittingNds:IsaevDimka\RussianPost\TariffInfo:private] => 0
       )
       */
  }
...
declare(strict_types=1);
  ...
  try {
      $otpravkaApi = new OtpravkaApi(Yaml::parse(file_get_contents('path_to_config.yaml')));
      $res = $otpravkaApi->getDeliveryPeriod(\IsaevDimka\RussianPost\PostType::EMS, 115551, 115551);
...
declare(strict_types=1);
  ...
  $recepient = new \IsaevDimka\RussianPost\Entity\Recipient();
...
use Symfony\Component\Yaml\Yaml;
use IsaevDimka\RussianPost\Providers\OtpravkaApi;
use IsaevDimka\RussianPost\Entity\Order;
...
try {
    $otpravkaApi = new OtpravkaApi(Yaml::parse(file_get_contents('path_to_config.yaml')));
    $result = $otpravkaApi->getPostOfficeFromPassport(\IsaevDimka\RussianPost\Enum\OpsObjectType::OPS);
    /*
    GuzzleHttp\Psr7\UploadedFile Object
    (
...
$otpravkaApi = new OtpravkaApi(Yaml::parse(file_get_contents('path_to_config.yaml')));
$result = $otpravkaApi->returnShipment(123456, \IsaevDimka\RussianPost\Enum\MailType::UNDEFINED);
...
try {
    ...
    $addressFrom = new \IsaevDimka\RussianPost\Entity\AddressReturn();
    $addressFrom->setAddressType(\IsaevDimka\RussianPost\Enum\AddressType::DEFAULT);
    $addressFrom->setIndex(125009);
    $addressFrom->setPlace('Москва');
    $addressFrom->setRegion('Москва');

    $addressTo = new \IsaevDimka\RussianPost\Entity\AddressReturn();
    $addressTo->setAddressType(\IsaevDimka\RussianPost\Enum\AddressType::DEFAULT);
    $addressTo->setIndex(115551);
    $addressTo->setPlace('Москва');
    $addressTo->setRegion('Москва');

    $return_shipment = new \IsaevDimka\RussianPost\Entity\ReturnShipment();
    $return_shipment->setMailType(\IsaevDimka\RussianPost\Enum\MailType::UNDEFINED);
