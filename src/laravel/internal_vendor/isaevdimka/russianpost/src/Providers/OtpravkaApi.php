...
use IsaevDimka\RussianPost\AddressList;
use IsaevDimka\RussianPost\Entity\Order;
use IsaevDimka\RussianPost\Entity\Recipient;
use IsaevDimka\RussianPost\Entity\ReturnShipment;
use IsaevDimka\RussianPost\Enum\OpsObjectType;
use IsaevDimka\RussianPost\Exceptions\RussianPostException;
use IsaevDimka\RussianPost\FioList;
use IsaevDimka\RussianPost\ParcelInfo;
use IsaevDimka\RussianPost\PhoneList;
use IsaevDimka\RussianPost\TariffInfo;
...
class OtpravkaApi implements LoggerAwareInterface
{
    ...
    public const DELIVERY_VERSION = 'v2';

    public const PRINT_TYPE_PAPER = 'PAPER';

    public const PRINT_TYPE_THERMO = 'THERMO';

    public const PRINT_ONE_SIDED = 'ONE_SIDED';

    public const PRINT_TWO_SIDED = 'TWO_SIDED';

    public const DOWNLOAD_FILE = 1;

    public const PRINT_FILE = 2;

    public const OTPRAVKA = 1; // Endpoint отправки

    public const DELIVERY = 2; // Endpoint сроков доставки

    public const POSTOFFICE = 3; // Endpoint работы с ОПС
    ...
    {
        return $this->callApi('GET', 'backlog/search', [
            'query' => $order_num,
        ]);
    }
    ...
    {
        return $this->callApi('GET', 'shipment/search', [
            'query' => $rpo,
        ]);
    }
    ...
    public function getAllBatches($mail_type = null, $mail_category = null, $size = null, $sort = 'ask', $page = null)
    {
        ...
        if (! empty($mail_category)) {
            $params['mailCategory'] = $mail_category;
        }
        if (! empty($size)) {
            $params['size'] = $size;
        }
    ...
    {
        return $this->callApi('GET', 'unloading-passport/zip', [
            'type' => $type,
        ]);
    }
    ...
    private function callApi($type, $method, $params = [], $version = self::V1, $endpoint = null)
    {
        ...
        switch ($type) {
            case 'GET':
                ...
                $response = $client->get($version . '/' . $method, [
                    'query' => $params,
                ]);
                break;
            case 'POST':
            ...
            case 'DELETE':
                ...
                $response = $client->{strtolower($type)}($version . '/' . $method, [
                    'json' => $params,
                ]);
                break;
        }
