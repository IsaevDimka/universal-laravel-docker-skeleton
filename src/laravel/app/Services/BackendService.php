...
{
    public const STATUS_FILE_NAME = 'health.json';

    public const SERVICE_OPERATIONAL = 'Operational';

    public const SERVICE_DOWN = 'Down';

    public const SERVICE_PAUSE = 'Pause';

    public const SERVICE_UPDATE = 'Update';
    ...
    protected string $latest_release;

    protected string $laravel_version;
    ...
    public function getHealth(): array
    {
        try {
            ...
            try {
                ...
                } elseif ($horizon_is_paused = collect($horizon)->contains(function ($master) {
...
//            }
    ...
    {
        $data = json_encode($this->getHealth(), JSON_PRETTY_PRINT);
    ...
    public function uptimeDocker(): string
    {
        ...
        $uptime = '';
        if (($years = $diff->y) > 0) {
            $uptime .= "${years} " . trans_choice('carbon.years', $years) . ', ';
        }
        if (($months = $diff->m) > 0) {
            $uptime .= "${months} " . trans_choice('carbon.months', $months) . ', ';
        }
        if (($days = $diff->d) > 0) {
            $uptime .= "${days} " . trans_choice('carbon.days', $days) . ', ';
        }
        if (($hours = $diff->h) > 0) {
            $uptime .= "${hours} " . trans_choice('carbon.hours', $hours) . ', ';
        }
        if (($minutes = $diff->i) > 0) {
            $uptime .= "${minutes} " . trans_choice('carbon.minutes', $minutes);
        }
        if (($seconds = $diff->s) > 0 && $minutes < 1) {
            $uptime .= "${seconds} " . trans_choice('carbon.seconds', $seconds);
        }
