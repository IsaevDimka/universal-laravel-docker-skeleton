...
class MailMessageNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        ...
        if (!empty($this->data['subject'])) $message->subject($this->data['subject']);
        if (!empty($this->data['replyTo'])) $message->replyTo($this->data['replyTo']);
        if (!empty($this->data['line_1'])) $message->line($this->data['line_1']);
        if (!empty($this->data['line_2'])) $message->line($this->data['line_2']);
        if (!empty($this->data['action_label']) || !empty($this->data['action_url'])) $message->action($this->data['action_label'], $this->data['action_url']);
        if (!empty($this->data['line_3'])) $message->line($this->data['line_3']);
